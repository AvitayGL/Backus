...
contains
    subroutine Boundary_mirror_3d_calculate (this, v_quantity, coordinates, angle, edge_num, is_offload)
        ...
        real(8) :: pc_i1, pc_inxp, pc_j1, pc_jnyp, pc_k1, pc_knzp
        ...
        pc_i1   = v_quantity%boundary_params%plane_const_i1
        pc_inxp = v_quantity%boundary_params%plane_const_inxp
        pc_j1   = v_quantity%boundary_params%plane_const_j1
        pc_jnyp = v_quantity%boundary_params%plane_const_jnyp
        pc_k1   = v_quantity%boundary_params%plane_const_k1
        pc_knzp = v_quantity%boundary_params%plane_const_knzp

        nxp = v_quantity%d1
        nyp = v_quantity%d2
        nzp = v_quantity%d3
        if (wall_x_bot .eqv. .true.) then
            !$omp target teams distribute parallel do collapse(2) if(is_offload_local)
            do kk = 0, nzp + 1
                do jj = 0, nyp + 1
